...
app.post('/passengers', (req, res) => {
  ...
  const insertQueries = passengers.map(passenger => {
    return new Promise((resolve, reject) => {
      const { given_name, nationality, gender, email, birthDate, travel_class, flight_number, gate } = passenger;
      const query = 'INSERT INTO passengers (given_name, nationality, gender, email, birthDate, travel_class, flight_number, gate) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
      db.query(query, [given_name, nationality, gender, email, birthDate, travel_class, flight_number, gate], (err, result) => {
        if (err) {
